...
(function(){
  ...
  function advancedTyreFilters(){
    ...
    const search=$('#tyre-product-search'), tierBtns=$$('[data-tyre-tier-filter]'), count=$('[data-tyre-result-count]'), status=$('#tyre-search-status');
    let tier='all';
    tierBtns.forEach(b=>b.addEventListener('click',()=>{tier=b.dataset.tyreTierFilter||'all';setTimeout(run,0)}));
    ...
    if(reset) reset.addEventListener('click',()=>{Object.values(controls).forEach(c=>{if(c)c.value='all'}); if(search)search.value=''; tier='all'; tierBtns.forEach(b=>b.classList.toggle('active',(b.dataset.tyreTierFilter||'all')==='all')); run();});
    ...
    function run(){
      ...
      if(status && status.lastChild) status.lastChild.textContent=shown===1?' tyre available':' tyres available';
    }
    run();
  }
