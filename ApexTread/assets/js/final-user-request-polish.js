...
(function(){
  ...
  if(search&&cards.length){
    ...
    function apply(){
      ...
      cards.forEach(card=>{const category=(card.dataset.category||'').toLowerCase();const hay=(card.textContent+' '+category).toLowerCase();const ok=(active==='all'||category===active)&&(!q||hay.includes(q));card.classList.toggle('hidden',!ok);card.hidden=!ok;card.setAttribute('aria-hidden',ok?'false':'true');if(ok)visible++;});
